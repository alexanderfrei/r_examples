function (data, xcoding, demos, prior, mcmc, constraints, options) 
{
  ...
  cat("", fill = TRUE)
  cat(npar, " parameters to be estimated", ifelse(none, " (including 'None').", 
                                                  "."), sep = "", fill = TRUE)
  cat("", fill = TRUE)
  ...
  cat(ifelse(share, sum(ytab[1, ]), sum(nsets)), ifelse(share, 
                                                        " expanded", ""), " tasks in total.", sep = "", fill = TRUE)
  cat("", fill = TRUE)
  ...
  cat("Constraints ", ifelse(constrain, "", "not "), "in effect.", 
      fill = TRUE, sep = "")
  cat("Draws ", ifelse(save, "are to be ", "not "), "saved.", 
      fill = TRUE, sep = "")
  ...
  for (rep in 1:R) {
    ...
    if (rep == 1) {
      yl = c(-1, 1)
      matplot(0, 0, type = "l", col = 1:8, lty = 1, ylim = yl, 
              xlim = c(1, R + fR), xlab = "Rep", ylab = "Mu")
    }
    if (rep%%100 == 0) {
  ...
  cat("Total Time Elapsed: ", (ctime - itime)%/%60, ifelse(round((ctime - 
                                                                    itime)%%60) > 9, ":", ":0"), round((ctime - itime)%%60), 
      fill = TRUE, sep = "")
  cat("", fill = TRUE)
  write.table(cbind(R + fR, nunits, npar, s, as.numeric(constrain), 
                    as.numeric(drawdelta)), "restart.txt", sep = " ", row.names = FALSE, 
              col.names = FALSE)
  write.table(oldbetas, "restart.txt", sep = " ", row.names = FALSE, 
              col.names = FALSE, append = TRUE)
  if (constrain) {
    write.table(oldbetas.c, "restart.txt", sep = " ", row.names = FALSE, 
                col.names = FALSE, append = TRUE)
  }
  if (drawdelta) {
    write.table(t(olddelta), "restart.txt", sep = " ", row.names = FALSE, 
                col.names = FALSE, append = TRUE)
  }
  betaout = betaout/use
